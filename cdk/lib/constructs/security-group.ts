...
import { createDefaultSecurityGroup, addIngressRule, STAGES, ALLOWED_HTTP_PORT, ALLOWED_HTTPS_PORT } from '@genflowly/cdk-commons';
...
export class AuthenticationServiceSecurityGroup extends Construct {
  ...
  constructor(scope: Construct, id: string, props: { vpc: Vpc }) {
    ...
    addIngressRule(this.securityGroup,Port.tcp(ALLOWED_HTTPS_PORT), 'Allow HTTPS traffic');
    //add port 22 for ssg
    addIngressRule(this.securityGroup, Port.tcp(22), 'Allow SSG traffic');
  }
}